...
class CRNN(torch.nn.Module):
    ...
    out_shape,
    n_targert=13,
    f_pool_szie=[4,4, 2], # CNN frequency pooling, length of list = number of CNN layers, list value = pooling per layer
    ...
    fnn_size=128,             # FNN contents, length of list = number of layers, list value = number of nodes
        ...
        if len(f_pool_size):
            for conv_cnt in range(len(f_pool_size)):
                self.conv_block_list.append(
                    ConvBlock(
                        in_channels=nb_cnn2d_filt if conv_cnt else in_feat_shape[1],
                        out_channels=nb_cnn2d_filt
                    )
                )
                self.conv_block_list.append(
                    torch.nn.MaxPool2d((t_pool_size[conv_cnt], f_pool_size[conv_cnt]))
                )
                self.conv_block_list.append(
        ...
        if nb_rnn_layers:
            self.in_gru_size = nb_cnn2d_filt * int( np.floor(in_feat_shape[-1] / np.prod(f_pool_size)))
        ...
        self.attn = None
        if self_attn:
#            self.attn = AttentionLayer(rnn_size, rnn_size, rnn_size)
